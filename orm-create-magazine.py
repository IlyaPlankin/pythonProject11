...
#customers = session.query(Customer.username== 'Fortioneaks').all()

#customers = session.query(Customer).filter(Customer.username.notin_(['Fortioneaks', 'Caltin1962','morozovkm'])).all

customers = session.query(Customer).all()
for customer in customers:
    print(customer.email, customer.first_name, customer.id)


customer_count=session.query(Customer).filter (Customer.username == 'Fortioneaks').count()
print(customer_count)
items = session.query(Item).filter(Item.name.like('%o')).limit(2).all()
for item in items:
    print(item.name)
    items = session.query(Item).filter(Item.name.like('%o')).limit(2).offset(2).all()
    for item in items:
        print(item.name)

customers_orders = session.query(Customer).join(Order).all()
for customers_order in customers_orders:
    print(customers_orders.username)
    for ord in customers_order.orders:
        print(ord.date_paced)
